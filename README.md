...
학교 취업지원팀과의 인터뷰를 통해 "학생들이 면접 연습 기회 자체가 부족하고, 자소서 피드백을 받기까지 시간이 오래 걸린다"는 실제 현장 수요를 확인하고 기획한 프로젝트입니다. 단순 아이디어성 과제가 아니라, 취업지원팀이 실무에서 겪는 병목(1:1 면접 코칭 인력 부족, 자소서 첨삭 대기 시간)을 AI로 보완하는 것을 목표로 설계했습니다.

## 시스템 아키텍처

```
[React 프론트엔드]
      │  REST API (JWT 인증, FormData/JSON)
      ▼
[Spring Boot 백엔드]  ← 인증/사용자/도메인 로직, 중재자 역할
      │  WebClient (내부 통신)
      ▼
